      ...
      <section id="give" style={{ background: 'linear-gradient(135deg, var(--navy) 0%, var(--navy-mid) 50%, var(--navy-light) 100%)', padding: 'clamp(64px,10vw,100px) clamp(16px,5vw,24px)', position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ maxWidth:900, margin:'0 auto', textAlign:'center', position:'relative' }}>
          ...
          <h2 style={{ fontFamily:'Playfair Display,serif', fontSize:'clamp(28px,5vw,52px)', color:'white', marginBottom:20, lineHeight:1.2 }}>
            Partner With <span className="accent-shimmer">Heaven's Mission</span>
          </h2>
          ...
          <div style={{ display:'flex', gap:'clamp(12px,3vw,20px)', justifyContent:'center', flexWrap:'wrap' }}>
            <button onClick={() => setShowOffering(true)} className="btn-primary" style={{ padding:'clamp(14px,3vw,18px) clamp(28px,6vw,48px)', fontSize:'clamp(12px,2vw,15px)' }}>
              Give an Offering
            </button>
        ...
        <div style={{ maxWidth:1100, margin:'0 auto', display:'grid', gridTemplateColumns:'repeat(auto-fit, minmax(min(100%,340px),1fr))', gap:'clamp(40px,8vw,80px)' }}>
          <div>
            ...
            <h2 style={{ fontFamily:'Playfair Display,serif', fontSize:'clamp(28px,5vw,44px)', color:'var(--navy)', marginBottom:24, lineHeight:1.2 }}>
              Connect With <span className="accent-text">Our Ministry</span>
            </h2>
            <p style={{ fontFamily:'Cormorant Garamond,serif', fontSize:'clamp(15px,2.5vw,19px)', color:'var(--text-mid)', lineHeight:1.85, marginBottom:'clamp(28px,5vw,40px)' }}>
              Whether you need prayer, want to partner, or simply want to connect — Heaven's table is always open. Reach us through any of the channels below.
            </p>
            <div style={{ display:'flex', flexDirection:'column', gap:'clamp(14px,3vw,20px)' }}>
              {[
                { icon:'', label:'WhatsApp', value:'+27 763 511 196', href:`https://wa.me/27763511196`, color:'#25D366' },
                { icon:'', label:'Direct Call', value:'+234 913 868 8465', href:`tel:+2349138688465`, color:'var(--orange)' },
                { icon:'', label:'TikTok', value:'@heavenshospitality', href:'https://www.tiktok.com/@heavenshospitality', color:'var(--navy)' },
                { icon:'', label:'Ministry', value:'Global — Reaching the Nations', href:null, color:'var(--text-light)' },
              ].map(({ icon, label, value, href, color }) => (
                <div key={label} style={{ display:'flex', gap:16, alignItems:'flex-start' }}>
                  <div style={{ width:44, height:44, background:'var(--off-white)', border:'1.5px solid rgba(11,26,46,0.1)', borderRadius:'50%', display:'flex', alignItems:'center', justifyContent:'center', fontSize:18, flexShrink:0, boxShadow:'var(--shadow)' }}>{icon}</div>
                  <div>
                    ...
                    {href ? (
                      <a href={href} target={href.startsWith('http')?'_blank':undefined} rel="noopener" style={{ fontFamily:'Cormorant Garamond,serif', fontSize:'clamp(15px,2.5vw,18px)', color:'var(--text-dark)', textDecoration:'underline', textDecorationColor:'rgba(232,76,14,0.3)' }}>{value}</a>
                    ) : (
          ...
          <div>
            <div style={{ background:'var(--navy)', borderRadius:12, padding:'clamp(28px,5vw,40px)', color:'white', marginBottom:20 }}>
              <div style={{ fontSize:'clamp(28px,6vw,36px)', textAlign:'center', marginBottom:16 }}></div>
              ...
              <p style={{ fontFamily:'Cormorant Garamond,serif', fontStyle:'italic', fontSize:'clamp(15px,2.5vw,17px)', color:'rgba(255,255,255,0.7)', textAlign:'center', lineHeight:1.75, marginBottom:24 }}>
                For prayer requests, partnership enquiries, or just to say hello — we're available on WhatsApp.
              </p>
              <a href="https://wa.me/27763511196" target="_blank" rel="noopener" style={{ display:'flex', alignItems:'center', justifyContent:'center', gap:10, background:'#25D366', color:'white', fontFamily:'Montserrat,sans-serif', fontSize:13, fontWeight:700, letterSpacing:1.5, textTransform:'uppercase', padding:'15px 24px', borderRadius:8, textDecoration:'none', transition:'all 0.2s' }}>
                Message on WhatsApp
              </a>
            </div>
            <div style={{ background:'var(--orange-pale)', border:'1.5px solid rgba(232,76,14,0.2)', borderRadius:12, padding:'clamp(20px,4vw,28px)' }}>
              <h4 style={{ fontFamily:'Playfair Display,serif', fontSize:'clamp(18px,3vw,22px)', color:'var(--navy)', marginBottom:10 }}>Prayer Line</h4>
              <p style={{ fontFamily:'Cormorant Garamond,serif', fontSize:'clamp(15px,2vw,17px)', color:'var(--text-mid)', lineHeight:1.7, marginBottom:14 }}>
                Call us directly for urgent prayer needs. Our team is here for you.
              </p>
              <a href="tel:+2349138688465" style={{ fontFamily:'Playfair Display,serif', fontSize:'clamp(18px,3vw,22px)', color:'var(--orange)', fontWeight:700, textDecoration:'none' }}>+234 913 868 8465</a>
            </div>
          </div>
        </div>
      </section>
