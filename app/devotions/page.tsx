    ...
    <>
      ...
      <main>
        ...
        <div style={{ background: 'linear-gradient(160deg, var(--navy), var(--navy-mid))', padding: '140px 24px 80px', textAlign: 'center' }}>
          <div className="section-label" style={{ marginBottom: 16 }}>Daily Devotions</div>
                  ...
                  {devotions.map(d => (
                    <Link key={d.id} href={`/devotions/${d.id}`} style={{ textDecoration: 'none' }}>
                      <div className="card-ministry" style={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
                        <div style={{ background: 'linear-gradient(135deg, var(--navy), var(--navy-light))', padding: '24px', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                          <span style={{ fontFamily: 'Montserrat, sans-serif', fontSize: 10, letterSpacing: 2, color: 'var(--orange)', textTransform: 'uppercase' }}>
                            {new Date(d.published_at).toLocaleDateString('en-US', { weekday: 'long', month: 'long', day: 'numeric' })}
                          </span>
                          <span style={{ color: 'rgba(232,76,14,0.6)', fontSize: 22 }}></span>
                        </div>
                        <div style={{ padding: '24px', flex: 1 }}>
                          <h3 style={{ fontFamily: 'Playfair Display, serif', fontSize: 22, marginBottom: 10, color: 'var(--navy)', lineHeight: 1.3 }}>{d.title}</h3>
                          <div style={{ fontFamily: 'Montserrat, sans-serif', fontSize: 11, color: 'var(--orange)', letterSpacing: 1, marginBottom: 14 }}> {d.scripture}</div>
                          <p style={{ fontFamily: 'Cormorant Garamond, serif', fontSize: 17, color: 'var(--text-mid)', lineHeight: 1.8 }}>
                            {d.content.replace(/<[^>]+>/g, '').slice(0, 150)}…
                          </p>
                        </div>
                        <div style={{ padding: '16px 24px', borderTop: '1px solid rgba(232,76,14,0.1)', fontFamily: 'Montserrat, sans-serif', fontSize: 11, letterSpacing: 2, color: 'var(--orange)', textTransform: 'uppercase' }}>
                          Read Full Devotion →
                        </div>
                      </div>
                    </Link>
