  ...
  return (
    <AdminLayout title="📖 Devotions">
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:28 }}>
        ...
        <Link href="/admin/devotions/new" className="btn-primary">+ New Devotion</Link>
      </div>
      ...
      {loading ? (
        ...
        <div style={{ textAlign:'center', padding:'60px 0', background:'rgba(255,255,255,0.02)', borderRadius:4, border:'1px dashed rgba(212,175,55,0.2)' }}>
          ...
          <Link href="/admin/devotions/new" className="btn-primary">Create Your First Devotion</Link>
        </div>
      ) : (
        <div style={{ display:'flex', flexDirection:'column', gap:12 }}>
          {devotions.map(d => (
            <div key={d.id} style={{ background:'rgba(255,255,255,0.04)', border:'1px solid rgba(212,175,55,0.12)', borderRadius:4, padding:'20px 24px', display:'flex', alignItems:'center', justifyContent:'space-between', gap:16, flexWrap:'wrap' }}>
              <div style={{ flex:1, minWidth:200 }}>
                ...
                <p style={{ fontFamily:'Montserrat,sans-serif', fontSize:11, color:'rgba(212,175,55,0.6)' }}>
                  {d.scripture && `📖 ${d.scripture} · `}
                  {new Date(d.published_at).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}
                </p>
              </div>
              <div style={{ display:'flex', gap:10, flexShrink:0 }}>
                <button onClick={() => sendNewsletter(d.id)} style={{ padding:'8px 14px', background:'rgba(212,175,55,0.1)', border:'1px solid rgba(212,175,55,0.3)', borderRadius:2, color:'#d4af37', fontFamily:'Montserrat,sans-serif', fontSize:11, cursor:'pointer' }}>
                  📧 Send
                </button>
