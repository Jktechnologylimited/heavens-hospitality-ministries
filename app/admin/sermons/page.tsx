  ...
  return (
    <AdminLayout title="🎙️ Sermons">
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:28 }}>
        ...
        <Link href="/admin/sermons/new" className="btn-primary">+ Add Sermon</Link>
      </div>
      ...
      {loading ? (
        ...
        <div style={{ textAlign:'center', padding:'60px 0', background:'rgba(255,255,255,0.02)', borderRadius:4, border:'1px dashed rgba(212,175,55,0.2)' }}>
          ...
          <Link href="/admin/sermons/new" className="btn-primary">Add Your First Sermon</Link>
        </div>
      ) : (
        <div style={{ display:'flex', flexDirection:'column', gap:12 }}>
          {sermons.map(s => (
            <div key={s.id} style={{ background:'rgba(255,255,255,0.04)', border:'1px solid rgba(212,175,55,0.12)', borderRadius:4, padding:'20px 24px', display:'flex', alignItems:'center', justifyContent:'space-between', gap:16, flexWrap:'wrap' }}>
