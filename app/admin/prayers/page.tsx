    ...
    <AdminLayout title="🙏 Prayer Requests">
      ...
      {loading ? (
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:16 }}>
          {prayers.map(p => (
            <div key={p.id} style={{ background:'rgba(255,255,255,0.04)', border:'1px solid rgba(212,175,55,0.12)', borderRadius:4, padding:'24px' }}>
              <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:12 }}>
                <div>
                  <span style={{ fontFamily:'Playfair Display,serif', fontSize:17, color:'white' }}>{p.name}</span>
                  {p.email && <span style={{ fontFamily:'Montserrat,sans-serif', fontSize:11, color:'rgba(212,175,55,0.6)', marginLeft:12 }}>{p.email}</span>}
              ...
              <p style={{ fontFamily:'Cormorant Garamond,serif', fontSize:17, color:'rgba(255,255,255,0.78)', lineHeight:1.75, fontStyle:'italic', borderLeft:'2px solid rgba(212,175,55,0.3)', paddingLeft:16 }}>
                {p.request}
              </p>
            </div>
          ))}
